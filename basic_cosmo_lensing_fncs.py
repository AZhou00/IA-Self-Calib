def get_basic_cosmo_lensing(d_horiz,Omega0,OmegaLambda,Omegak,z_list,outputpath):
    ...
    # W_L_list[z_L,z_G], shape(len(z_list),len(z_list))
...
def get_integrated_lensing_kernel(W_L_list,true_z_file,z_list,z_bin_num,outputpath):  ###(CHECKED)###
    ...
    for bin_index in range(z_bin_num):
        ...
        for z_L_index in range(len(z_list)):
            W_i[bin_index,z_L_index] = sum(W_L_list[z_L_index]*n_bin)*z_spacing #using the true redshift distribution
